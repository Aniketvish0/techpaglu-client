...
function App() {
  ...
  return (
    ...
    <Navbar/>
    <div className="mt-16 flex items-center justify-center p-4 z-10">
      <div className="grid grid-cols-1 md:grid-cols-5 w-full gap-8 max-w-6xl">
         <div className="md:col-span-2 flex justify-center w-full">
          <div className="max-w-md w-full bg-gray-900/90 border border-green-500/30 rounded-xl shadow-xl p-6 md:p-8 backdrop-blur-sm">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-center text-green-400 mb-2">
              Real TechPaglu ?
            </h1>
            ...
            <form onSubmit={analyzeHandle} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="block w-full pl-8 pr-3 py-2 md:py-3 bg-gray-800 border rounded-lg outline-none focus-visible:border-green-400 text-white text-sm md:text-base"
              ...
              <button
                ...
                className={`w-full py-2 md:py-3 px-4 rounded-lg text-black font-medium transition text-sm md:text-base
                  ${
              ...
              >
                {loading ? (
                  <span className="flex items-center justify-center">
                    ...
                    <span className="text-xs md:text-sm">Analyzing... (this may take up to 30 seconds)</span>
                  </span>
                ) : (
                  'Analyze Tweets'
                )}
            ...
            {result && (
              <div className="mt-8 text-center">
                <div className="mb-4 relative p-4 md:p-6 border border-green-500/30 rounded-xl bg-gray-800/70 backdrop-blur-sm">
                  ...
                  <div className={`text-xl md:text-2xl font-bold ${getResultColor(getCategory(result.tech_enthusiasm_score))}`}>
                    You are{' '}
                    <span
                      className={
                  ...
                  {result.key_tech_interests && result.key_tech_interests.length > 0 && (
                    <div className="mt-6">
                      <h3 className="text-sm md:text-md font-medium text-green-400 mb-3">
                      ...
                      <div className="flex flex-wrap justify-center gap-2">
                        {result.key_tech_interests.map((interest, index) => (
                          <span
                            key={index}
                            className="px-2 py-1 md:px-3 md:py-1 bg-green-900/50 border border-green-500/30 text-green-400 text-xs rounded-full"
                          >
                            {interest}
                          </span>
                        ))}
                  ...
                  <button
                    ...
                    className="mt-6 flex items-center justify-center gap-2 mx-auto py-2 px-4 bg-blue-500 hover:bg-blue-600 rounded-lg text-white font-medium transition disabled:bg-blue-800 disabled:opacity-70 text-sm md:text-base"
                  >
                    {isSharingLoading ? (
                      <>
            ...
            <div className="mt-6 pt-6 border-t border-green-500/20 text-center text-xs text-gray-500">
              score your tech enthusiasm and share with friends!
            </div>
          </div>
        </div>
        <div className="md:col-span-3 w-full max-h-[80vh] overflow-y-auto">
            <Leaderboard/>
          </div>
        </div>
      </div>
